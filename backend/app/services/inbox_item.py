...
async def list_inbox_items(db: AsyncSession) -> list[InboxItem]:
    result = await db.execute(select(InboxItem).order_by(InboxItem.created_at))
    return list(result.scalars().all())


async def get_inbox_item(db: AsyncSession, inbox_item_id: uuid.UUID) -> InboxItem | None:
    return await db.get(InboxItem, inbox_item_id)
...
async def delete_inbox_item(db: AsyncSession, obj: InboxItem, *, surface: str = "api") -> None:
    before = model_to_dict(obj)
    entity_id = obj.id
    await db.delete(obj)
    await db.flush()
