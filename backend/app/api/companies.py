...
async def list_companies(
    ...
    total = await svc.count_companies(db, q=q)
    set_pagination_headers(response, total=total, page=page)
    return await svc.list_companies(db, q=q, limit=page.limit, offset=page.offset)
...
async def get_company_by_slug(slug: str, db: AsyncSession = Depends(get_db)):  # noqa: B008
    obj = await svc.get_company_by_slug(db, slug)
    if obj is None:
